...
- 画面順序は「登録・確認 → 店舗選択 → 店舗のカテゴリ別商品選択 → 店内マップ → 購入内容確認・完了」。店舗を先に選び、その棚に設定済みかつ未購入商品が存在するカテゴリだけを表示する。
- 登録・編集は同じダイアログを利用する。自動分類は候補提示のみ。未分類を明示的に保存できる。
- 既定の12カテゴリに加えて、世帯共有の手動カテゴリを `households/{hid}/categoryCatalog/active` に保存する。`names` はカテゴリIDと表示名のマップ、`lastEditedId` は今回変更したID。ルールは1カテゴリずつの追加・名称変更だけを許可する。既存カテゴリの削除は許可しない。最大200追加カテゴリ、名前50文字、1棚50カテゴリ。
- カタログを1文書にまとめることで、棚の複数カテゴリ検証に必要なルールの文書参照回数を抑える。他世帯のカタログ閲覧・利用は不可。
- 買い物対象とマップのチェックは、世帯・店舗ごとの端末内セッション。Firestoreの商品状態は完了画面で確定するまで変更しない。アプリの終了でセッションはリセットされるが、登録商品は残る。
- 購入確定時は、選択かつチェック済みで、その店の対象カテゴリに属する現存商品のIDだけを削除する。400件単位の冪等バッチで、失敗時は残った商品の再試行が可能。未チェック・未選択の商品は残す。旧バージョンの `isPurchased` は互換性のため保持し、購入済み商品は登録画面から除外する。
- 新しいメンバーは作成・参加時に表示名を入力する。既存メンバーは「わが家」で自分の名前だけを変更できる。旧「あなた」表示は個人を区別できる仮表示へ置き換え、買いたい商品の追加者は `addedByUid` と世帯メンバー情報で表示する。
